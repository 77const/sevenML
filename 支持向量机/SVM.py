...
class SMO:
    ...
    def train(self, X, Y, K=L, C=1.0, tol=1e-3, max_passes=5):
        ...
        print('Done with K_cache', file=sys.stderr)
        ...
        while passes < max_passes:
            ...
            for i in range(m):
                ...
                if (y_i * E_i < -tol and alpha_i < C) or (y_i * E_i > tol and alpha_i > 0.0):
                    ...
                    b_i = b - E_i - y_i * (alpha_i - alpha_ii) * K_cache[i, i] - y_j * (alpha_j - alpha_jj) * K_cache[
                        i, j]
                    b_j = b - E_j - y_i * (alpha_i - alpha_ii) * K_cache[i, j] - y_j * (alpha_j - alpha_jj) * K_cache[
                        j, j]
...
if __name__ == '__main__':
    ...
    print('Training accuracy for Pegasos : %lf%%' % (100 * acc_train), file=sys.stderr)
    print('Test accuracy for Pegasos : %lf%%' % (100 * acc_test), file=sys.stderr)
    ...
    print('Training accuracy for SMO with linear kernel : %lf%%' % (100 * acc_train), file=sys.stderr)
    print('Test accuracy for SMO with linear kernel : %lf%%' % (100 * acc_test), file=sys.stderr)
    ...
    print('Training accuracy for SMO with rbf kernel : %lf%%' % (100 * acc_train), file=sys.stderr)
    print('Test accuracy for SMO with rbf kernel : %lf%%' % (100 * acc_test), file=sys.stderr)
